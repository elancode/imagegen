...
app.post('/auth/login', async (req, res) => {
    try {
        ...
        // Include user's models in the response
...
app.post('/train', authenticateToken, upload.array('images', 20), async (req, res) => {
    try {
        ...
        // Save training ID to user's account
...
app.post('/generate', authenticateToken, async (req, res) => {
    try {
        ...
        // Retrieve the fine-tuned model's version ID
        const modelName = req.user.models.find(m => m.modelId === modelId)?.name;
        if (!modelName) {
        ...
        const modelDetailsResponse = await axios.get(
            `https://api.replicate.com/v1/models/elancode/${modelName}`,
            {
                headers: {
                    'Authorization': `Token ${process.env.REPLICATE_API_TOKEN}`,
                },
